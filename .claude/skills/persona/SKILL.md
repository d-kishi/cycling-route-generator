...
Collector返却値に対し、checklist全カテゴリの要約が揃っているかを機械的に確認する。
配置: Collector返却直後。

- 不足カテゴリがある場合: 次ループで追加収集する指示を作り、Collectorに再投入する（ループ残があれば）
...
| チェックリスト完全性 | Collector成果物 | Collector返却直後 | 不足カテゴリを提示、再収集 | メインAgentによる照合（スクリプト化は将来） |
